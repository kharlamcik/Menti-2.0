...
export default function Room({ data: { text, inviteCode } }) {
  ...
  return (
    <section className="relative mx-auto flex w-full max-w-7xl flex-1 flex-col items-center justify-center px-2">
      <div className="mb-10 rotate-3 rounded-md bg-white px-6 py-4 text-6xl font-extrabold hover:scale-105 transition-all duration-300">
        {inviteCode}
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        {playerList.map((player) => (
          <div
            key={player.id}
            className="shadow-inset rounded-md bg-primary px-4 py-3 font-bold text-white hover:scale-105 transition-all duration-300"
